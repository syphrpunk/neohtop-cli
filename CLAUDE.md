...
`ts/` hosts the in-progress TypeScript port built on [incur](https://github.com/wevm/incur) (wevm's agent-first CLI framework) — see `ts/README.md` for port status. It reuses the Go `--json` field names so outputs stay compatible. Commands: `system`, `processes`, `top`, `tree`, `proc`, `snapshot`, `watch` (streaming), `kill`, plus persistence: `record`/`history` (SQLite metrics store via `bun:sqlite`), `config`, and `service install|status|uninstall` (hourly launchd recorder). State lives in `~/.config/neohtop/`. All commands support `--format toon|json|yaml|md|jsonl`, `--llms`, and MCP. Native bun APIs only (`bun:ffi` for libproc, `bun:sqlite` for storage) — no dependencies beyond incur. TUI will eventually use OpenTUI (low priority).
